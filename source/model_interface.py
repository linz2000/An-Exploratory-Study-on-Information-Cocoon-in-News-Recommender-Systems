...
from dataset import NewsDataset, UserDataset, BaseDataset, FMDataset, FM_Content_Dataset, NGCF_Dataset
...
from train_fm_content import train as train_fm_content_model
from train_fm_content import test_fm_content_model
...
class FM_Rec_Interface():
    def __init__(self, checkpoint_dir):
        ...
        if model_name == 'NCF' or model_name == 'DFM_ID':
    ...
    def getChickProb(self, user_feat, recall_news_feat): 
        """
        Args:
            user_feat: batch_size of user feat list
            recall_news_feat: batch_size x candidate_size of item feat list
        Returns:
...
class NGCF_Rec_Interface():
    ...
    def getNews2Emb(self, news2int: dict): 
    ...
    def getUser2Emb(self, user2int: dict): 
    ...
    def getChickProb(self, news_vector, user_vector):
        """
        Args:
            news_vector: batch_size, candidate_size, embedding_dim
...
class FM_Content_Rec_Interface():
    def __init__(self, checkpoint_dir):

        if not model_name.endswith('_ONLY'):
            self.field_dims = np.array(MINDConfig.field_dims)
        else:
            self.field_dims = np.array(MINDConfig.id_field_dims)
        ...
        try:
            if not model_name.endswith('_ONLY'):
                ...
                Model = getattr(importlib.import_module(f"model.FM.{model_name[:-5].lower()}"), model_name[:-5])
        except AttributeError:
            print(f"{model_name} not included!")
            exit()

        if model_name == 'DFM_CONTENT' or model_name == 'DFM_CONTENT_ONLY':
            self.model =  Model(self.field_dims, embed_dim=16, mlp_dims=(16, 16), dropout=0.2, conti_dim=MINDConfig.news_emb_dim).to(device)
    ...
    def retrain(self, items, targets, news_int2emb): 
        dataset = FM_Content_Dataset.init_from_list(items, targets, news_int2emb)
        data_loader = DataLoader(dataset, batch_size=FMConfig.batch_size, num_workers=8)
        ...
        train_fm_content_model(self.model, optimizer, data_loader, criterion, device)
    ...
    def evaluate(self, fm_behaviors): 
        ...
        auc, acc, macro_f1, micro_f1 = test_fm_content_model(self.model, data_loader, device)

        return auc, acc, macro_f1, micro_f1

    def getNews2Emb(self, news2feat_id_list, news_int2emb: dict): 
        feat_id_arr = []
        news_list = []
        news_emb_arr_list = []
        for news, feat_id_list in news2feat_id_list.items():
            feat_id_arr.append(feat_id_list)
            news_list.append(news)
            news_emb_arr_list.append(news_int2emb[feat_id_list[0]])
        ...
        for i in range(math.ceil(news_list.__len__() / batch_size)):
            ...
            news_emb_arr_cut = news_emb_arr_list[i * batch_size: (i + 1) * batch_size]
            news_emb_arr_cut = np.array(news_emb_arr_cut)
            news_emb_tensor_cut = torch.FloatTensor(news_emb_arr_cut).to(device)

            with torch.no_grad():
                if not model_name.endswith('_ONLY'):
                    news_embs_cut = self.model.get_mixed_embs(feat_id_tensor_cut, MINDConfig.item_feat_idx, news_emb_tensor_cut)
                else:
                    news_embs_cut = self.model.get_mixed_embs(feat_id_tensor_cut, MINDConfig.item_idx, news_emb_tensor_cut)
    ...
    def getUser2Emb(self, user2feat_id_list):
        ...
        for i in range(math.ceil(user_list.__len__() / batch_size)):
            ...
            with torch.no_grad():
                user_embs_cut = self.model.get_embs(feat_id_tensor_cut, MINDConfig.user_feat_idx)
    ...
    def getChickProb(self, user_feat, recall_news_feat, news_int2emb): 
        """
        Args:
            recall_news_feat: batch_size x candidate_size of list
        ...
        for u_feat, news_feat_list in zip(user_feat, recall_news_feat):
            for news_feat in news_feat_list:
                input.append(u_feat + news_feat)
                news_emb_list.append(news_int2emb[news_feat[0]])

        input = np.array(input).astype(np.int)
        input = torch.LongTensor(input).to(device)

        news_emb_arr = np.array(news_emb_list)
        news_emb_tensor = torch.FloatTensor(news_emb_arr).to(device)

        with torch.no_grad():
            pred = self.model(input, news_emb_tensor) 
...
class Content_Rec_Interface():
    ...
    def getChickProb(self, news_vector, user_vector): 
        ...
        Args:
            ...
            user_vector: batch_size, (DKN: click_news_num, )word_embedding_dim
        Returns:
            click_probability: batch_size, candidate_size
        """
