...
try:
    if model_name in model_name in ['NRMS', 'NAML', 'DKN']:
...
class UserDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item = {
            ...
            "id":
            self.user2int[row.user] if row.user in self.user2int else 0 
        }
        item['clicked_news_length'] = len(item["clicked_news"])
...
class FM_Content_Dataset(): 
    def __init__(self, items, targets, news_int2emb: dict):
        self.items = items
        self.targets = targets
        self.news_int2emb = news_int2emb

    @classmethod
    def init_from_path(cls, behaviors_path, news2int_path, news2emb_path, feat_idx_list: list = None):
        data = pd.read_table(
            behaviors_path,
            header=0).to_numpy()
        if not feat_idx_list:
            items = data[:, :-1].astype(np.int)
        else:
            items = data[:, feat_idx_list].astype(np.int)
        ...
        for news, emb_arr in news2emb_arr.items():
            if news in news2int:
                news_int2emb[news2int[news]] = emb_arr

        return cls(items, targets, news_int2emb)

    @classmethod
    def init_from_list(cls, items, targets, news_int2emb: dict):
        items = np.array(items).astype(np.int)
        targets = np.array(targets).astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        news_emb = self.news_int2emb[item[1]]
        return item, news_emb, self.targets[index]

    def get_news_int2emb(self):
        return self.news_int2emb
...
class NGCF_Dataset(object):
    def __init__(self, dir, train_path, val_path, test_path, batch_size=NGCFConfig.batch_size):
        ...
        self.train_items, self.val_items, self.test_items = {}, {}, {} 
